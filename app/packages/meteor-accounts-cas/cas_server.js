 ...
 */
 Accounts.registerLoginHandler(function (options) {
  ...
  var user = Accounts.updateOrCreateUserFromExternalService("cas", result, options);
   ...
   Accounts.setUsername(user.userId, result.id);
   Accounts.addEmail(user.userId, result.id.concat("@hawaii.edu"), true);

   if (_.contains(Meteor.settings.admins, result.id)) {
